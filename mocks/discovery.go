...
func (_m *Discovery) JoinNodes(nodes ...string) error {
	...
	if len(ret) == 0 {
		panic("no return value specified for JoinNodes")
	}
...
func (_m *Discovery) SendEvent(name string, payload []byte, params *clustertool.QueryParam) (*serf.QueryResponse, error) {
	...
	var r1 error
	if rf, ok := ret.Get(0).(func(string, []byte, *clustertool.QueryParam) (*serf.QueryResponse, error)); ok {
		return rf(name, payload, params)
	}
	if rf, ok := ret.Get(0).(func(string, []byte, *clustertool.QueryParam) *serf.QueryResponse); ok {
